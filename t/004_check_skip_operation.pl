...
# segment $num_read_records / 2 times

my $num_read_records = 4;

for (my $iter = 0; $iter < int($num_read_records / 2); $iter++) 
{
    my $read_record;
...
for (my $i = 0; $i < int($num_read_records / 2); $i++) 
{
    ...
    if ($current_position % $block_size == 0) 
    {
        ...
        # Compare headers
        $is_equal = substrings_equal($current_position, $hdr_len, $current_position, $current_position);
...
my $current_waldiff_position = $current_position;

# Skip all bytes in WAL segment, that not presented in WALDIFF segment

for (my $i = int($num_read_records / 2); $i < int($num_read_records / 2) + $num_skip_records; $i++) 
{   
    my $rec_maxaligned_len = @record_maxaligned_lengths[$i];
    my $rec_len            = @record_lengths[$i];
    my $hdr_len            = 0;
...
for (my $i = int($num_read_records / 2) + $num_skip_records; $i < $num_skip_records + $num_read_records; $i++) 
{
    my $rec_maxaligned_len = @record_maxaligned_lengths[$i];
    my $rec_len            = @record_lengths[$i];
    my $hdr_len            = 0;
