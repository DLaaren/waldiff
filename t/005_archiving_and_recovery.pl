...
# Primary node, doing archives with WALDIFFs
...
# Standby node from backup, fetching WALDIFFs from primary node
...
# diag("result primary: " . $result_primary);
# diag("result standby: " . $result_standby);

# is($result_standby, qq(1000), 'check content from archives');
# is($result_primary, qq(2000), 'check content from archives');
