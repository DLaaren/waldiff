				   ...
				   XLogRecord *record)
{
	...
	while (true) {
		...
		if (*record_len > free_page_space)
		{
			...
			// ereport(LOG, errmsg("just write: %lu %lu", MAXALIGN(*record_len), writer->writeBufSize));
			break;
		}
...
WaldiffWriteBufferToDisk(WaldiffWriter *writer)
{
	...
	written_bytes = pg_pwrite(writer->seg.ws_file, (void*) writer->writeBuf, writer->writeBufSize, writer->WriteRecPtr % writer->segcxt.ws_segsize);
	pgstat_report_wait_end();

	if (written_bytes != XLOG_BLCKSZ)
		ereport(ERROR,
			(errcode_for_file_access(),
			errmsg("could not write to file \"%d\": %m", writer->seg.ws_file)));
	if (written_bytes == 0)
		ereport(ERROR,
			(errcode_for_file_access(),
